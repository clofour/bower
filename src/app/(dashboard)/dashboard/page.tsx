...
export default async function OverviewPage() {
  ...
  return <div className="mx-auto max-w-7xl"><PageHeading eyebrow="Control plane" title={`Good ${new Date().getHours() < 12 ? 'morning' : 'afternoon'}, ${user.name.split(' ')[0]}.`} description="A focused view of what is shipping, what is healthy, and what needs your attention." />
    ...
    ].map(([Icon, label, value, detail]) => { const C = Icon as typeof FolderKanban; return <Card key={String(label)} className="relative overflow-hidden p-5"><div className="absolute -right-8 -top-8 h-24 w-24 rounded-full bg-primary/5" /><C className="h-4 w-4 text-primary" /><p className="mt-6 text-3xl font-bold tracking-tight">{String(value)}</p><p className="mt-1 text-sm font-bold">{String(label)}</p><p className="mt-1 text-xs text-muted-foreground">{String(detail)}</p></Card> })}</div>
