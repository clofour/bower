...
export default async function IntegrationsPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <div className="space-y-6"><div><h2 className="text-lg font-bold">Integrations</h2><p className="text-sm text-muted-foreground">Signed inbound deployments and outbound status delivery.</p></div>
    ...
    <Card className="flex items-center justify-between p-5"><div className="flex items-center gap-3"><Radio className="h-5 w-5 text-primary" /><div><h3 className="font-bold">Real-time Trellis event stream</h3><p className="text-sm text-muted-foreground">Canopy polls while Trellis SSE is unavailable.</p></div></div><NoopButton feature="Trellis real-time events">Unavailable</NoopButton></Card>
  </div>
}
