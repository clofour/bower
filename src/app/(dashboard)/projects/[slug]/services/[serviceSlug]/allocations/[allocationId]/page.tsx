...
export default async function AllocationPage({ params, searchParams }: {
  ...
  return <div className="space-y-6">
    ...
    <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-medium uppercase tracking-widest text-primary">Allocation</p><h2 className="mt-2 font-mono text-2xl font-semibold">{allocation.id}</h2><p className="mt-1 text-sm text-muted-foreground">{allocation.address ?? 'No node assigned'} · revision {allocation.job_revision}</p></div><div className="flex gap-2"><Status value={allocation.phase} /><Status value={allocation.health} /></div></div>
